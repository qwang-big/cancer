...
cnv_obj <- inferCNV(data = expr_tr,
                    ...
                    window_size = 101,
                    out_path = "output_dir_new", # dir is auto-created for storing outputs
                    noise_filter = NULL,
                    vis_bounds = "-0.5,0.5")
save(cnv_obj,file = paste0(args[1],".infered.cnv.rda"))
...
save(cnv_obj,file = paste0(args[1],".res.cnv.rda"))
load('LungCancer_T_FO14.robj')
load('FO14_CNV_result_new_0.5_new.Robj')
tree = as.data.frame(cutree(cnv_obj$cluster$hclust,k = 3))
colnames(tree) = 'Cluster'
tree$barcode = rownames(tree)
tree = tree[cnv_obj$target,]
Tumor = tree[tree$Cluster==3,]$barcode
Tumor = Tumor[gsub('_.*','',Tumor) == 'Tumor']
seurat_spatialObj@meta.data$cnv='low'
seurat_spatialObj@meta.data[gsub('.*_','',Tumor),]$cnv='high'
SpatialDimPlot(seurat_spatialObj,group.by = 'cnv',pt.size.factor = 2.5)
