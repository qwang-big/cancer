...
def make_annot_files(args, bed_for_annot):
    ...
    df_bim = pd.read_csv(args.bimfile, dtype=str,
                         delim_whitespace=True, usecols=[0, 1, 2, 3], names=['CHR', 'SNP', 'CM', 'BP'])
    iter_bim = [['chr'+str(x1), x2, x2]
    ...
    df_int = pd.DataFrame({'BP': bp, 'ANNOT': 1}, dtype=str)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--annot-file', type=str,
                        help='the name of the annot file to output.')
